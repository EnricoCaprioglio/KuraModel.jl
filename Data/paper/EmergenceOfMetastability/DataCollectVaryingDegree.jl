...
# set folder path
folderpath = "/mnt/lustre/scratch/inf/ec627/data/HierarchicalChimera/paper_data/DataCollect/n1608Var_k/"
...
for k in k_range
    ...
    KOP_modules_mean = zeros(B[1])
    KOP_modules_std = zeros(B[1])

    for i in 1:B[1]
        KOP_modules[:, i] = macro_op(θs[relax:save_ratio:end, n[1]*(i-1)+1:n[1]*i])
    end

    KOP_modules_mean = mean(KOP_modules, dims = 1)
    KOP_modules_std = std(KOP_modules, dims = 1)
    ...
    # save results
    results = [KOP_whole_mean, KOP_whole_std, KOP_pop_1_mean, KOP_pop_1_std, KOP_pop_2_mean, KOP_pop_2_std, KOP_modules_mean, KOP_modules_std, KOP_modules, params]
